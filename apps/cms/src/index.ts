...
export default {
  ...
  async bootstrap({ strapi }: { strapi: Core.Strapi }) {
    ...
    // Auto-grant public read permissions for non-production environments so web/preview
    // can render without admin setup. Defaults to enabled when NODE_ENV !== 'production'.
    // Explicitly override via STRAPI_AUTO_PUBLIC_READ=true|false.
    const autoPublicReadEnv = process.env.STRAPI_AUTO_PUBLIC_READ;
    const autoPublicRead =
      autoPublicReadEnv === 'true' ||
      (autoPublicReadEnv !== 'false' && process.env.NODE_ENV !== 'production');
    ...
    if (autoPublicRead) {
      try {
        ...
        if (publicRole?.id) {
          ...
          strapi.log.info('[bootstrap] ensured public read permissions for non-production env');
        }
      } catch (err) {
