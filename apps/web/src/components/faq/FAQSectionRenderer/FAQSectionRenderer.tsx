        ...
        <>
            {sections.map((section, index) => {
                switch (section.__component) {
                    case 'sections.hero':
                        return <FAQHero key={index} data={section} app={app} />;
                    case 'sections.tutorial-accordion':
                        return <FAQAccordion key={index} data={section} app={app} />;
                    case 'sections.cta':
                        return <FAQCTA key={index} data={section} />;
                    default:
                        return null;
                }
            })}
...
const FAQHero: React.FC<{ data: HeroSection; app: App }> = ({ data, app }) => (
    <section className="pt-24 pb-16 bg-section-bg-3 text-center">
        <div className="container-custom max-w-[1000px]">
            <h1 className="text-white text-[28px] md:text-[48px] font-black text-heading leading-[1.2] mb-6 tracking-tight">
                {data.title || `F.A.Q. of ${app.name}`}
            </h1>

            <RichText
                content={data.subtitle}
                className="text-white/80 text-[18px] leading-relaxed mb-10 max-w-[800px] mx-auto font-light"
            />
            ...
            <div className={cn(
                ...
                <div className="overflow-hidden">
                    <div className="px-12 pb-12">
                        ...
                        {isOpen && hasAdSenseSlot('faqAccordion') ? (
                            <div className="mt-10 border-t border-gray-50 pt-10">
                                <PageAdSlot
                                    placement="faqAccordion"
                                    minHeight={250}
                                />
                            </div>
                        ) : null}
...
const FAQCTA: React.FC<{ data: CTASection }> = ({ data }) => (
    <section className="pt-20 bg-white text-center border-t border-gray-50 mt-12">
